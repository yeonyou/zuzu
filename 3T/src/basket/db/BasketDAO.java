...
public class BasketDAO {
	...
	public Vector getBasketList(String id) {
		...
		String sql = "select * from basket_3t where BASKET_MEMBER_ID =?";
		...
		try {
			...
			while(rs.next()) {
				...
				sql = "select * from goods_3t where goods_num=?";
				pstmt = conn.prepareStatement(sql);
	...
	public void basketAdd(String id, int goodsnum, int amount, String size, String color) {
		String sql="select max(basket_num) from basket_3t";
		int num=0;
		...
		try {
			...
			sql="insert into basket_3t values "+"(?,?,?,?,?,?,sysdate)";
	...
	public boolean basketRemove(int num) {
		String sql = "delete from basket_3t where BASKET_NUM=?";
