...
public class OrderDAO {
	...
	public int getOrderCount(String id) throws SQLException {
		String sql = "select count(*) from goods_order_3t where order_member_id=?";
		try {
	...
	public int getOrderSumMoney(String id) throws SQLException {
		String sql = "select sum(order_sum_money) from goods_order_3t where order_member_id=? ";
		try {
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, id);
			rs = pstmt.executeQuery();
	...
	public List getOrderList(int page, int limit, String id) throws SQLException {
		String sql = "select * from (select rownum rnum,ORDER_GOODS_NUM,"
				...
				+ "ORDER_GOODS_COLOR,ORDER_SUM_MONEY,ORDER_DATE," + "ORDER_STATUS from (select * from GOODS_ORDER_3t "
				+ "where ORDER_MEMBER_ID=? order by ORDER_DATE desc)) " + "where rnum>=? and rnum<=?";
	...
	public int addOrder(OrderBean order, Vector goodsvector) {
		...
		int ordernum = 0;
		String sql = "select max(ORDER_NUM) from goods_order_3t";
		try {
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			rs.next();
		...
		for (int i = 0; i < basketlist.size(); i++) {
			...
			try {
				...
				sql = "insert into goods_order_3t values(?,?,?,?,?," + "?,?,?,?,?,?,?,?,?,?,?,?,sysdate,?,sysdate,0)";
